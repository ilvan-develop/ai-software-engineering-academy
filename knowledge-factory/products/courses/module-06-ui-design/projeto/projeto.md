...
| Ferramenta | Uso |
|-----------|-----|
| Figma Inspect | Medir elementos, copiar CSS |
| Pixel Perfect (Chrome) | Overlay do design sobre o código |
| VisBug | Editar design no browser |
| Storybook | Catálogo de componentes |
| Chromatic | Review visual e regressão |
| Percy | Screenshot diff |
...
1. **UI Design** é a camada visual da experiência — cores, tipografia, componentes
2. **CRAP**: Contraste, Repetição, Alinhamento, Proximidade
3. **Cor**: teoria, paletas funcionais, acessibilidade (4.5:1), branding
4. **Tipografia**: hierarquia, escalas modulares, legibilidade
5. **Espaçamento e Grid**: escalas consistentes, sistemas de coluna, layouts
6. **Componentes**: botões, inputs, cards, modais, tabelas, dropdowns
7. **Design Patterns**: navegação, formulários, feedback, onboarding, empty states
8. **Microinterações**: hover, active, focus, disabled, animações
9. **Dark Mode**: tema alternativo com paleta própria, contraste ajustado
10. **UI para devs**: inspeção no Figma, fidelidade, densidade, estados
