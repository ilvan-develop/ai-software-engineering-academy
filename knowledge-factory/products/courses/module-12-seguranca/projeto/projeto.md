...
1. **Segurança é pré-requisito** — não é feature, não é opcional
2. **OWASP Top 10** — conheça as 10 vulnerabilidades mais comuns
3. **Autenticação JWT** — access + refresh token com rotação
4. **Autorização CASL** — abilities granulares por role
5. **Rate Limiting** — protege contra brute force e abuso
6. **Helmet + CSP** — headers de segurança obrigatórios
7. **Proteções específicas** — SQL injection, XSS, CSRF
8. **Secrets Management** — nada de senhas no código
9. **Auditoria** — logs de ações sensíveis para rastreabilidade
