...
1. **NestJS** — melhor framework Node.js para Enterprise (DI, módulos, guard, interceptors)
2. **Controllers** — roteamento e validação de entrada
3. **Services** — lógica de negócio e orquestração
4. **Repositories** — separação de persistência (DDD)
5. **DTOs + Zod** — validação na porta de entrada
6. **Exception Filters** — tratamento global de erros
7. **Interceptors + Guards** — transformação e proteção de rotas
8. **Paginação cursor-based** — escala melhor que offset
9. **Cache com Redis** — performance em dados frequentes
10. **Health Checks** — observabilidade da aplicação
11. **Testes** — unitários + integração com mocks
