...
1. **Server Components por padrão** — menos JS no cliente, mais performance
2. **Data fetching no servidor** — sem useEffect, sem loading states complexos
3. **Loading + Error + Empty** — todo componente deve ter os 3 estados
4. **Server Actions** — mutations sem API routes
5. **Zustand** — estado global simples e tipado
6. **TanStack Query** — cache do servidor no cliente
7. **Acessibilidade** — ARIA, teclado, foco visível, labels
8. **Mobile-first** — Tailwind breakpoints do menor para o maior
9. **Performance** — next/image, next/font, dynamic import
