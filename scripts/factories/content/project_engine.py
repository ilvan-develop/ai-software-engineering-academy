...
"""
project_engine.py
Gera mini-projeto completo a partir de aula.md.
Inclui enunciado, criterios, template de entrega e rubrica de avaliacao.
"""
...
def extract_code_languages(text: str) -> list:
    langs = re.findall(r"```(\w+)", text)
    return list(set(langs))


def generate_project(title: str, headings: list, objectives: list, languages: list) -> str:
    ...
    lang = languages[0] if languages else "TypeScript"

    lines = [
        f"# Projeto Pratico — {title}",
        "",
        "---",
        "",
        "## Visao Geral",
        "",
        f"**Tema:** {topic}",
        f"**Duracao estimada:** 8-12 horas",
        f"**Dificuldade:** Intermediario/Avancado",
        f"**Linguagem:** {lang}",
        "",
        "---",
        "",
        "## Contexto",
        "",
        f"Voce foi contratado como arquiteto de software para construir um sistema enterprise que aplica os conceitos de **{topic}**. "
        f"Este projeto simula um cenario real onde voce precisa tomar decisoes arquiteturais, implementar solucoes e documentar suas escolhas.",
        "",
        f"**Cenario:** Uma empresa de medio porte precisa modernizar sua plataforma. "
        f"O sistema atual e legado e nao atende mais as necessidades de escalabilidade e manutencao. "
        f"Sua missao e projetar e implementar uma nova solucao seguindo as melhores praticas do modulo.",
        "",
        "---",
        "",
        ...
        f"2. Deve implementar corretamente os conceitos de {sub_topics[0] if sub_topics else topic}",
        ...
        "|----------|------|-------------------|-----------------|-------------------|",
        f"| Correcao tecnica | 30% | Nao implementa {sub_topics[0] if sub_topics else 'conceitos'} corretamente | Implementa mas com falhas | Implementacao robusta e completa |",
...
def main():
    parser = argparse.ArgumentParser(description="Gerador de Projetos Praticos")
    parser.add_argument("--input", required=True, help="Caminho para aula.md")
    parser.add_argument("--output", required=True, help="Pasta de saida (projeto/)")
    ...
    languages = extract_code_languages(text)

    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)

    project = generate_project(title, headings, objectives, languages)
    project_path = output_dir / "projeto.md"
    project_path.write_text(project, encoding="utf-8")
    print(f"  OK Projeto: {project_path}")

    print(f"  [DONE] Projeto gerado para: {title}")
