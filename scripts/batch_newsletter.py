"""batch_newsletter.py — Gera newsletters por curso. Usa cache incremental."""
...
def _course_sources(course_dir: Path) -> list[Path]:
    """Return all aula.md paths for a course."""
...
def main():
    ...
    for course_dir in courses:
        ...
        if run(engine, course_dir, course, out):
            ...
            ok += 1
    print(f"\n---\nNewsletters: {ok}/{total} cursos", end="")
    if skipped:
