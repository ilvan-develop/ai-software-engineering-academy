...
"""
batch_projects.py
Processa projetos para todos os modulos.
...
def main():
    ...
    engine = SCRIPTS_DIR / "factories" / "content" / "project_engine.py"
    cache = BuildCache()
    ...
    for course_name, mods in modules.items():
        for mod_id, mod in mods.items():
            ...
            output_dir = COURSES_DIR / course_name / mod_id / "projeto"
            item_id = f"{course_name}/{mod_id}"

            if not cache.is_stale(item_id, "projeto", sources=[aula_path]):
                skipped += 1
                continue
            ...
            if run_engine(engine, aula_path, output_dir):
                mod.setdefault("outputs", {})["projeto"] = "gerado"
                cache.mark_done(item_id, "projeto", sources=[aula_path])
                ok += 1
            else:
                print(f"  FALHOU")

    save_status(data)
    print(f"\n---\nProjetos: {ok}/{total} processados", end="")
    if skipped:
