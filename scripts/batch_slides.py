...
"""
batch_slides.py
Processa slides para todos os modulos.
Le o status.yaml, encontra modulos pendentes,
e dispara slide_engine.py para cada um.
...
def main():
    ...
    engine = SCRIPTS_DIR / "factories" / "content" / "slide_engine.py"
    cache = BuildCache()
    ...
    for course_name, mods in modules.items():
        for mod_id, mod in mods.items():
            ...
            output_dir = COURSES_DIR / course_name / mod_id / "slides"
            item_id = f"{course_name}/{mod_id}"

            if not cache.is_stale(item_id, "slides", sources=[aula_path]):
                skipped += 1
                continue
            ...
            if run_engine(engine, aula_path, output_dir):
                ...
                cache.mark_done(item_id, "slides", sources=[aula_path])
                ok += 1
            else:
                print(f"  FALHOU")

    save_status(data)
    print(f"\n---\nSlides: {ok}/{total} processados", end="")
    if skipped:
